...
function Nav() {
  ...
  return (
    <header className="relative z-10 h-[67px] border-b border-[#f1f3f7] bg-white max-[980px]:h-16">
      ...
      >
        <a
          className="inline-flex shrink-0 items-center max-[980px]:absolute max-[980px]:left-1/2 max-[980px]:-translate-x-1/2"
          href="#home"
          onClick={closeMenu}
        ...
        <div className="ml-[250px] flex flex-1 items-center justify-between max-[980px]:hidden">
          <div className="flex items-center gap-[31px]">
            {links.map((link) => (
              <a className={`text-[14px] font-semibold leading-none no-underline transition hover:text-[#f23893] ${link === 'Home' ? 'text-[#f23893]' : 'text-[#64748b]'}`} href={`#${link.toLowerCase()}`} key={link}>
                {link}
              </a>
            ))}
          </div>
          <div className="flex items-center gap-5">
            <a className={signInClasses} href="#sign-in">Sign In</a>
            <a className={signUpClasses} href="#sign-up">Sign Up</a>
          </div>
        </div>

        <button
          className="order-first hidden h-10 w-10 place-items-center rounded-lg border-0 bg-transparent p-0 text-[#172033] hover:bg-[#f8f2f7] max-[980px]:inline-grid max-[520px]:h-9 max-[520px]:w-9"
          type="button"
        ...
        <div className="ml-auto hidden items-center gap-4 max-[980px]:flex max-[520px]:gap-[clamp(7px,2.5vw,13px)]">
          <a className={signInClasses} href="#sign-in">Sign In</a>
          <a className={signUpClasses} href="#sign-up">Sign Up</a>
        </div>
        ...
        <div className={`absolute right-0 left-0 top-[calc(100%-1px)] hidden flex-col overflow-hidden border-b bg-white px-4 opacity-0 shadow-[0_12px_20px_rgb(15_23_42_/_0%)] transition-[max-height,opacity,visibility,box-shadow] duration-200 max-[980px]:flex ${menuOpen ? 'visible max-h-[390px] border-[#edf0f5] py-[10px] pb-[18px] opacity-100 shadow-[0_12px_20px_rgb(15_23_42_/_8%)]' : 'invisible max-h-0 border-transparent py-0'}`}>
          {links.map((link) => (
            <a className={`flex min-h-[42px] items-center overflow-hidden px-2 text-[14px] font-semibold leading-none no-underline transition hover:text-[#f23893] ${link === 'Home' ? 'text-[#f23893]' : 'text-[#64748b]'}`} href={`#${link.toLowerCase()}`} key={link} onClick={closeMenu}>
              {link}
            </a>
          ))}
        </div>
